...
if (!isPresent) {
	...
	return // Termina l'esecuzione dello script
			...
			//filtro per importo e cliente
			WebUI.setText(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 17 - Filtri - corretto filtraggio per Cliente e Importo/Importo'),Importo)
			WebUI.setText(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 17 - Filtri - corretto filtraggio per Cliente e Importo/Cliente'),Cliente)
			...
			// Trova tutte le righe all'interno della tabella
				...
				String Importo2 = columns1.get(2).getText()
				println Importo2
				
				//dal dettaglio pratica del primo record estraggo i valori sotto
				WebUI.click(findTestObject('Object Repository/DASHBOARD_FILTRI/4 - Filtri - corretto aggiornamento stato pratica nel dettaglio - da creata a pagata/Page_PlickUp-Admin/Apri_dettaglio'))
				String Cliente2 = WebUI.getText(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 17 - Filtri - corretto filtraggio per Cliente e Importo/Dettaglio_cliente'))
				println(Cliente2)
				...
				for (int k = 0; k < rows1.size(); k++) {
					...
					if (columns2.size() > 1) {
						...
						String Importo1 = columns2.get(2).getText()
						println Importo1
						WebUI.verifyMatch(Importo1, Importo, true)
