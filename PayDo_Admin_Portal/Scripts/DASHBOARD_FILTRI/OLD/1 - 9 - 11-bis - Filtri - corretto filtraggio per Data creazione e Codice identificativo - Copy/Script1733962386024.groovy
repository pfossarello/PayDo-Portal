...
// Naviga tra le pagine finché il pulsante "Avanti" è abilitato
		while (true) {
			...
			// Trova tutte le righe all'interno della tabella
				...
				LocalDate dataScadenzaDate = LocalDate.parse(dataScadenza, formatter1)
				...
				// Controlla se la dataScadenzaDate è nel range tra firstDayDate e lastDayDate
				if (dataScadenzaDate.isBefore(firstDayDate1) || dataScadenzaDate.isAfter(lastDayDate1)) {
					WebUI.comment("data_creazione ${dataScadenza} NON è nel range tra ${firstDayStr} e ${lastDayStr}")
					WebUI.verifyEqual(false, true) // Fai fallire il test
				} else {
					WebUI.comment("data_creazione ${dataScadenza} è nel range tra ${firstDayStr} e ${lastDayStr}")
				}
				...
				for (int k = 0; k < rows1.size(); k++) {
					...
					if (columns2.size() > 1) {
						...
						String dataScadenza1 = columns2.get(6).getText()
						...
						// Controlla se NON siamo all'ultima riga della tabella
							if (k < rows1.size() - 1) { // Se non è l'ultima riga
									...
									// Parsea la data di creazione nel formato "dd/MM/yyyy"
									LocalDate dataScadenzaDate2 = LocalDate.parse(dataScadenza1, formatter1)
									...
									LocalDate lastDayDate2 = LocalDate.parse(lastDayStr, formatter1)    // ultima  data (data odierna) dell'intervallo selezionato nel calendar
											...
											// Controlla se la dataScadenzaDate è nel range tra firstDayDate e lastDayDate
											if (dataScadenzaDate2.isBefore(firstDayDate2) || dataScadenzaDate2.isAfter(lastDayDate2)) {
												WebUI.comment("data_creazione ${dataScadenzaDate2} NON è nel range tra ${firstDayStr} e ${lastDayStr}")
												WebUI.verifyEqual(false, true) // Fai fallire il test
											} else {
												WebUI.comment("data_creazione ${dataScadenzaDate2} è nel range tra ${firstDayStr} e ${lastDayStr}")
											}
