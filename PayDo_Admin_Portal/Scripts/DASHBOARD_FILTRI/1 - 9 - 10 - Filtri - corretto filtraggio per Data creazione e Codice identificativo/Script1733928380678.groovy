...
if (!isPresent) {
	...
	return // Termina l'esecuzione dello script
			...
			//Esploro la tabella e prendo l'identificativo della prima riga da usare come filtro
			...
			////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////7
			// Apri il calendar
			WebUI.click(findTestObject('Object Repository/DASHBOARD_FILTRI/FILTRI APPOGGIO/Page_PlickUp-Admin/Data creazione'))
...
WebUI.setText(findTestObject('Object Repository/DASHBOARD_FILTRI/FILTRI APPOGGIO/Page_PlickUp-Admin/Codice identificativo'),identificativo)
...
// Naviga tra le pagine finché il pulsante "Avanti" è abilitato
		while (true) {
			...
			// Trova tutte le righe all'interno della tabella
				...
				String dataCreazione = columns1.get(1).getText()
				String Identificativo1 = columns1.get(0).getText()
				...
				LocalDate dataCreazioneDate = LocalDate.parse(dataCreazione, formatter1)
				...
				if (dataCreazioneDate.isBefore(firstDayDate1) || dataCreazioneDate.isAfter(lastDayDate1)) {
					WebUI.comment("data_creazione ${dataCreazione} NON è nel range tra ${firstDayStr} e ${lastDayStr}")
					WebUI.verifyEqual(false, true) // Fai fallire il test
				} else {
					WebUI.comment("data_creazione ${dataCreazione} è nel range tra ${firstDayStr} e ${lastDayStr}")
				}
			
				WebUI.verifyMatch(Identificativo1, identificativo, true)
				...
				for (int k = 0; k < rows1.size(); k++) {
					...
					if (columns2.size() > 1) {
						...
						String dataCreazione1 = columns2.get(1).getText()
						String Identificativo2 = columns2.get(0).getText()
						...
						// Controlla se NON siamo all'ultima riga della tabella
							if (k < rows1.size() - 1) { // Se non è l'ultima riga
									...
									// Parsea la data di creazione nel formato "dd/MM/yyyy"
									LocalDate dataCreazioneDate2 = LocalDate.parse(dataCreazione1, formatter1)
									...
									LocalDate lastDayDate2 = LocalDate.parse(lastDayStr, formatter1)    // ultima  data (data odierna) dell'intervallo selezionato nel calendar
											...
											if (dataCreazioneDate2.isBefore(firstDayDate2) || dataCreazioneDate2.isAfter(lastDayDate2)) {
												WebUI.comment("data_creazione ${dataCreazioneDate2} NON è nel range tra ${firstDayStr} e ${lastDayStr}")
												WebUI.verifyEqual(false, true) // Fai fallire il test
											} else {
												WebUI.comment("data_creazione ${dataCreazioneDate2} è nel range tra ${firstDayStr} e ${lastDayStr}")
											}
											WebUI.verifyMatch(Identificativo2, identificativo, true)
