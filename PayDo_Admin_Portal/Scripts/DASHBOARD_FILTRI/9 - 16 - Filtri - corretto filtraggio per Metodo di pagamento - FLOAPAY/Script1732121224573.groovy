...
WebUI.click(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 16 - Filtri - corretto filtraggio per Metodo di pagamento - FLOAPAY/span_Created_4'))
...
// Trova tutte le righe all'interno della tabella
	...
	//dal dettaglio pratica del primo record estraggo i valori sotto
		...
		String Metodo_pagamento = WebUI.getText(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 12 - Filtri - corretto filtraggio per Causale e Metodo di pagamento - PISP/Metodo pagamento'))
		println(Metodo_pagamento)
		WebUI.verifyMatch(Metodo_pagamento, "FLOAPAY", true)
			...
			// Controlla se NON siamo all'ultima riga della tabella
				if (k < rows1.size() - 1) { // Se non è l'ultima riga
					...
					WebUI.delay(1) // Aspetta che il dettaglio venga caricato
							String Metodo_pagamento1 = WebUI.getText(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 12 - Filtri - corretto filtraggio per Causale e Metodo di pagamento - PISP/Metodo pagamento'))
							println(Metodo_pagamento1)
							WebUI.verifyMatch(Metodo_pagamento1, "FLOAPAY", true)
