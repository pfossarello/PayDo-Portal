...
WebUI.click(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 25 - Filtri - corretto filtraggio per Stato e Data di passaggio e visualizzazione del dettaglio - Refused/span_Created_5'))
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////77
...
// Trova tutte le righe all'interno della tabella
	...
	data_refused = WebUI.getText(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 25 - Filtri - corretto filtraggio per Stato e Data di passaggio e visualizzazione del dettaglio - Refused/data cambio stato in refused'))
	println(data_refused)
	WebUI.delay(2)
	WebUI.click(findTestObject('Object Repository/DASHBOARD_FILTRI/4 - Filtri - corretto aggiornamento stato pratica nel dettaglio - da creata a pagata/Page_PlickUp-Admin/Apri_dettaglio'))
	
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//Controlla che data_refused sia nel range
	if (!data_refused) {
		WebUI.comment("data_rifiuto è vuota o null!")
		WebUI.verifyEqual(false, true) // Fai fallire il test
	}

	// Parsea la data di rifiuto nel formato "dd/MM/yyyy"
	LocalDate dataRefusedDate = LocalDate.parse(data_refused, formatter1)
	...
	// Controlla se la data_refused è nel range tra firstDayDate e lastDayDate
	if (dataRefusedDate.isBefore(firstDayDate1) || dataRefusedDate.isAfter(lastDayDate1)) {
	    WebUI.comment("data_rifiuto ${data_refused} NON è nel range tra ${firstDayStr} e ${lastDayStr}")
	    WebUI.verifyEqual(false, true) // Fai fallire il test
	} else {
	    WebUI.comment("data_rifiuto ${data_refused} è nel range tra ${firstDayStr} e ${lastDayStr}")
	}
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			// Controlla se NON siamo all'ultima riga della tabella
				if (k < rows1.size() - 1) { // Se non è l'ultima riga
					...
					data_refused = WebUI.getText(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 25 - Filtri - corretto filtraggio per Stato e Data di passaggio e visualizzazione del dettaglio - Refused/data cambio stato in refused'))
						...
						//Controlla che data_refused sia nel range
						if (!data_refused) {
							WebUI.comment("data_refused è vuota o null!")
							WebUI.verifyEqual(false, true) // Fai fallire il test
						}
										
						// Parsea la data di Rifiuto nel formato "dd/MM/yyyy"
						LocalDate dataRefusedDate2 = LocalDate.parse(data_refused, formatter1)
						...
						LocalDate lastDayDate2 = LocalDate.parse(lastDayStr, formatter1)    // ultima  data (data odierna) dell'intervallo selezionato nel calendar
								...
								// Controlla se la data_refused è nel range tra firstDayDate e lastDayDate
								if (dataRefusedDate2.isBefore(firstDayDate2) || dataRefusedDate2.isAfter(lastDayDate2)) {
								    WebUI.comment("data_rifiuto ${data_refused} NON è nel range tra ${firstDayStr} e ${lastDayStr}")
								    WebUI.verifyEqual(false, true) // Fai fallire il test
								} else {
								    WebUI.comment("data_rifiuto ${data_refused} è nel range tra ${firstDayStr} e ${lastDayStr}")
