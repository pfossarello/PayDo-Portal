...
// Trova tutte le righe all'interno della tabella
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//Controlla che data_invio sia nel range
	if (!data_invio) {
		WebUI.comment("data_invio è vuota o null!")
		WebUI.verifyEqual(false, true) // Fai fallire il test
	}

	// Parsea la data di invio nel formato "dd/MM/yyyy"
	LocalDate dataInvioDate = LocalDate.parse(data_invio, formatter1)
	...
	// Controlla se la data_invio è nel range tra firstDayDate e lastDayDate
	if (dataInvioDate.isBefore(firstDayDate1) || dataInvioDate.isAfter(lastDayDate1)) {
	    WebUI.comment("data_invio ${data_invio} NON è nel range tra ${firstDayStr} e ${lastDayStr}")
	    WebUI.verifyEqual(false, true) // Fai fallire il test
	} else {
	    WebUI.comment("data_creazione ${data_invio} è nel range tra ${firstDayStr} e ${lastDayStr}")
	}
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			// Controlla se NON siamo all'ultima riga della tabella
				if (k < rows1.size() - 1) { // Se non è l'ultima riga
					...
					data_invio = WebUI.getText(findTestObject('Object Repository/DASHBOARD_FILTRI/6 - Filtri - corretto aggiornamento stato pratica nel dettaglio - da inviata a pagata/data cambio stato in inviata'))
						...
						// Parsea la data di Invio nel formato "dd/MM/yyyy"
						LocalDate dataInvioDate2 = LocalDate.parse(data_invio, formatter1)
						...
						LocalDate lastDayDate2 = LocalDate.parse(lastDayStr, formatter1)    // ultima  data (data odierna) dell'intervallo selezionato nel calendar
								...
								// Controlla se la dataInvio è nel range tra firstDayDate e lastDayDate
								if (dataInvioDate2.isBefore(firstDayDate2) || dataInvioDate2.isAfter(lastDayDate2)) {
								    WebUI.comment("data_invio ${data_invio} NON è nel range tra ${firstDayStr} e ${lastDayStr}")
								    WebUI.verifyEqual(false, true) // Fai fallire il test
								} else {
								    WebUI.comment("data_invio ${data_invio} è nel range tra ${firstDayStr} e ${lastDayStr}")
								}
								///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
