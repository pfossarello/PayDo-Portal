...
WebUI.click(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 15 - Filtri - corretto filtraggio per Metodo di pagamento - PAYPALSUBSCRIPTION/span_Created_3'))
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
// Trova tutte le righe all'interno della tabella
	...
	data_payed = WebUI.getText(findTestObject('Object Repository/DASHBOARD_FILTRI/4 - Filtri - corretto aggiornamento stato pratica nel dettaglio - da creata a pagata/Page_PlickUp-Admin/data cambio stato in pagata'))
	println(data_payed)
	WebUI.delay(2)
	WebUI.click(findTestObject('Object Repository/DASHBOARD_FILTRI/4 - Filtri - corretto aggiornamento stato pratica nel dettaglio - da creata a pagata/Page_PlickUp-Admin/Apri_dettaglio'))
	
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//Controlla che data_payed sia nel range
	if (!data_payed) {
		WebUI.comment("data_pagamento è vuota o null!")
		WebUI.verifyEqual(false, true) // Fai fallire il test
	}

	// Parsea la data di pagamento nel formato "dd/MM/yyyy"
	LocalDate dataPayedDate = LocalDate.parse(data_payed, formatter1)
	...
	// Controlla se la data_payed è nel range tra firstDayDate e lastDayDate
	if (dataPayedDate.isBefore(firstDayDate1) || dataPayedDate.isAfter(lastDayDate1)) {
	    WebUI.comment("data_pagamento ${data_payed} NON è nel range tra ${firstDayStr} e ${lastDayStr}")
	    WebUI.verifyEqual(false, true) // Fai fallire il test
	} else {
	    WebUI.comment("data_pagamento ${data_payed} è nel range tra ${firstDayStr} e ${lastDayStr}")
	}
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			// Controlla se NON siamo all'ultima riga della tabella
				if (k < rows1.size() - 1) { // Se non è l'ultima riga
					...
					data_payed = WebUI.getText(findTestObject('Object Repository/DASHBOARD_FILTRI/4 - Filtri - corretto aggiornamento stato pratica nel dettaglio - da creata a pagata/Page_PlickUp-Admin/data cambio stato in pagata'))
						...
						//Controlla che data_payed sia nel range
						if (!data_payed) {
							WebUI.comment("data_payed è vuota o null!")
							WebUI.verifyEqual(false, true) // Fai fallire il test
						}
										
						// Parsea la data di Pagamento nel formato "dd/MM/yyyy"
						LocalDate dataPayedDate2 = LocalDate.parse(data_payed, formatter1)
						...
						LocalDate lastDayDate2 = LocalDate.parse(lastDayStr, formatter1)    // ultima  data (data odierna) dell'intervallo selezionato nel calendar
								...
								// Controlla se la dataPayed è nel range tra firstDayDate e lastDayDate
								if (dataPayedDate2.isBefore(firstDayDate2) || dataPayedDate2.isAfter(lastDayDate2)) {
								    WebUI.comment("data_creazione ${data_payed} NON è nel range tra ${firstDayStr} e ${lastDayStr}")
								    WebUI.verifyEqual(false, true) // Fai fallire il test
								} else {
								    WebUI.comment("data_creazione ${data_payed} è nel range tra ${firstDayStr} e ${lastDayStr}")
								}
								///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
