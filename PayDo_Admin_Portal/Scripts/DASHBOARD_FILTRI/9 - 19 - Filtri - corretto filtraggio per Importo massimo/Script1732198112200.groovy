...
WebUI.setText(findTestObject('Object Repository/DASHBOARD_FILTRI/9 - 19 - Filtri - corretto filtraggio per Importo massimo/Importo massimo'),Importo)
...
// Trova tutte le righe all'interno della tabella
	...
	WebUI.verifyLessThanOrEqual(importoRecuperato, Importo, FailureHandling.STOP_ON_FAILURE) // Se la condizione fallisce, il test si ferma
	...
	for (int k = 0; k < rows1.size(); k++) {
		...
		if (columns2.size() > 1) {
			...
			// Verifica se l'importo recuperato è maggiore o uguale all'importo
			WebUI.verifyLessThanOrEqual(importoRecuperato1, Importo, FailureHandling.STOP_ON_FAILURE) // Se la condizione fallisce, il test si ferma
